...
name: ddd-qa-chain
description: >
  Run the DDD quality assurance test chain—5 automated layers (domain logic /
  IPC contract / component interaction / E2E user journeys / visual regression)
  that verify a build equals its original definition and meets quality bar.
  Each layer has a tool, command, and gate; a run-all command is the delivery
  gate, plus a DoD quality gate (deterministic assertions + probabilistic
  sample/threshold for AI behavior). Use when delivering or verifying a feature, before claiming work
  complete, or when asked to run full self-tests / quality checks (DDD 测试链 /
  全链自测 / 交付前质量验证).
---

# DDD 质量保证测试链（ddd-qa-chain）

## 目标

**交付的产品 = 最初定义 + 质量达标。** 5 层自动化验证，每一层有工具、命令、门禁；全链通过才可交付。
...
**一项功能「Done」须同时满足（借鉴 Acceptance Criteria Patterns + DoD for AI Agents）：**
...
| 定义对齐 | 实现可追溯到最初定义（tickets AC ↔ 文档）| ④ 一致性 |

**铁律（AI 应用特化）**：确定性部分绝不用「模型应该会」糊弄——必须断言；概率部分绝不用「一次通过」冒充——要样本/阈值证据。

## 与相关技能协作

- `verification-before-completion`：证据先于断言（每层结果必须有命令输出佐证）
...
- `product-doc-audit`：④ 交付=定义（文档审计/就绪度）
