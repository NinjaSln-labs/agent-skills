---
name: sin-rules
description: 用户 sin 的全局通用规则，应在所有项目/会话中遵守。4 条：密码安全（禁明文凭证）、权限确认（写操作先确认）、长任务反馈（60s 内持续反馈）、会话进度（session.md 维护）。当涉及密码/凭证、写操作、长任务执行、会话记录，或任何项目开工/收工时适用。
---
...
每个项目独立维护自己的 `session.md`，放在 `<project>/.deepcode/session.md`。

工作区根目录的 `.deepcode/session.md` 仅存跨项目信息（子项目清单、全局待定方向），不存放任何单个项目的进度。

### 分层
- `<project>/.deepcode/session.md`：该项目活跃事项 + 待定方向 + 关键上下文（引用 `docs/` 路径即可，不重复记录）
